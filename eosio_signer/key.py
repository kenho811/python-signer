...
class EOSIOKey(Signer):
    def __init__(self, private_str=''):
        ''' '''
        if private_str:
            private_key, self._format, self._key_type = self._parse_key(private_str)
    ...
    def to_wif(self):
        ...
        if self._format == 'WIF':
            ...
            pri_key = hexlify(self._sk.to_string()).decode()
            return 'PVT_{}_{}'.format(self._key_type, self._check_encode(pri_key, self._key_type).decode())
